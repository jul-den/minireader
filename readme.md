...
Этот проект является форком оригинального [MiniReader](https://github.com/circlejourney/MiniReader) от [circlejourney](https://github.com/circlejourney). Огромное спасибо автору за создание такого чистого и полезного инструмента!
...
# Локализация файлов историй **(НОВОЕ)**

Теперь MiniReader поддерживает локализацию не только интерфейса, но и содержимого историй на уровне файлов.

## Файлы историй

При сканировании папки `stories/` система сначала ищет файл `story_{текущий_язык}.json`. Если он не найден, используется стандартный `story.json`. Это позволяет локализовать метаданные истории (название, описание) без дублирования структуры.

**Пример:**
- `stories/my-story/story.json` — описание на языке по умолчанию (например, английском)
- `stories/my-story/story_ru.json` — описание на русском языке (если есть)

## Главы

Аналогично работает с файлами глав. При запросе главы система сначала пытается прочитать `{номер}_глвы_{текущий_язык}.html`, а при его отсутствии — стандартный `{номер}.html`.

**Пример:**
- `stories/my-story/1.html` — глава на языке по умолчанию
- `stories/my-story/1_ru.html` — русский перевод этой же главы

Это даёт возможность переводить как метаданные, так и сам текст историй независимо друг от друга.

## Индикатор языка

В интерфейсе рядом с переключателем темы появился индикатор текущего языка в виде флага. Он подгружается через библиотеку [flag-icons](https://github.com/lipis/flag-icons) и автоматически отображает флаг текущего языка.

## Планы на будущее

В следующих версиях планируется добавить **интерактивный переключатель языков**, чтобы пользователь мог вручную выбрать язык интерфейса и содержимого, независимо от настроек браузера.
